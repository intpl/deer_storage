...
* TODO [61%] [49/80] DEADLINE: <2020-03-31 Tue>
...
** DONE Check if invitation can assign another subscription after changing inviting user's current one
    test use case: user invites invitee to subscription A, switches to B, invitee clicks -> what happens here?
** DONE navigation/sign in path fixes for administrators with subscription assigned
** TODO Changing email in registration does not validate email format
** TODO Changing email again applies previous one even if it is not in valid format. Crazy.
