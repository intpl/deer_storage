defmodule DeerStorageWeb.UserView do
  ...
  def link_to_toggle_permission(
        %{role: "admin"},
        _user_permissions_link,
        _permission_key,
        _can_manage?,
        _current_user
      ),
  ...
  def link_to_toggle_permission(user, user_permissions_link, permission_key, _can_manage?, %{
        role: "admin"
      }) do
  ...
  defp clickable_button(
         %{subscription_id: subscription_id} = user_permissions_link,
         permission_key,
         user_id,
         is_enabled
       ) do
    ...
    classes =
      if is_permitted?, do: "button is-light is-success", else: "button is-light is-danger"

    button(is_permitted? |> translated_yes_or_no,
      to:
        Routes.user_user_path(
