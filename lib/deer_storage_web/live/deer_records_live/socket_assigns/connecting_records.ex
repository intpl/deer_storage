defmodule DeerStorageWeb.DeerRecordsLive.Index.SocketAssigns.ConnectingRecords do
  ...
  import DeerStorage.DeerRecords,
    only: [connect_records!: 3, remove_orphans_from_connected_records!: 2]

  def assign_connecting_records_after_update(%{assigns: %{connecting_record: nil}} = socket, _),
    do: socket
  ...
  def assign_connecting_records_after_update(
        ...
        %{deer_table_id: table_id} = updated_record
      ) do
    new_records =
      try_to_update_record(records, updated_record) ||
        search_records(subscription_id, table_id, query, 1)
  ...
  def assign_connecting_records_after_delete(%{assigns: %{connecting_records: []}} = socket, _),
    do: socket
  ...
  def assign_connecting_records_after_delete(
    ...
    new_records =
      reduce_list_with_function(records, fn record -> unless(id == record.id, do: record) end)

    assign(socket, connecting_records: new_records)
  end

  def assign_connecting_records_after_delete(
        %{assigns: %{connecting_records: records, connecting_record: %{id: connecting_record_id}}} =
          socket,
        ids
      )
      when is_list(ids) do
    case Enum.member?(ids, connecting_record_id) do
      ...
      false ->
        assign(
          socket,
          :connecting_records,
          reduce_list_with_function(records, fn record ->
  ...
  def assign_closed_connecting_records(socket),
    do: assign(socket, connecting_record: nil, connecting_records: [], connecting_query: nil)

  def assign_filtered_connected_records(
        %{
          assigns: %{
            current_subscription: subscription,
            connecting_selected_table_id: old_table_id
          }
        } = socket,
        query,
        new_table_id
    ...
    if new_table_id != old_table_id,
      do:
        Enum.find(subscription.deer_tables, fn %{id: id} -> id == new_table_id end) ||
  ...
  def handle_connecting_records(
        %{
          assigns: %{
            opened_records: opened_records,
            connecting_record: connecting_record,
            connecting_records: connecting_records,
            current_subscription: subscription
          }
        } = socket,
        selected_record_id
      ) do
    [record1, _connected_records] =
      Enum.find(opened_records, fn [record, _connected_records] ->
        record.id == connecting_record.id
      end)
  ...
  def assign_modal_for_connecting_records(
        %{assigns: %{records: records, current_subscription: subscription, table_id: table_id}} =
          socket,
        record_id
      ) do
