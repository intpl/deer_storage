defmodule DeerStorageWeb.DeerRecordsLive.Index.SocketAssigns.UploadingFiles do
  ...
  import DeerStorageWeb.DeerRecordsLive.Index.SocketAssigns.Helpers,
    only: [any_entry_started_upload?: 1, find_record_in_opened_or_connected_records: 2]

  def assign_uploading_file_for_record_after_update(
        %{assigns: %{uploading_file_for_record: %{id: record_id}}} = socket,
        %{id: record_id} = updated_record
      ) do
    socket
    |> assign(:uploading_file_for_record, updated_record)
    |> reload_subscription_storage_and_allow_upload
  end

  def assign_uploading_file_for_record_after_update(socket, _),
  ...
  def assign_opened_file_upload_modal(
    ...
    uploading_record =
      find_record_in_opened_or_connected_records(opened_records, String.to_integer(record_id))

    socket
    |> reload_subscription_storage_and_allow_upload
    |> assign(:uploading_file_for_record, uploading_record)
  end

  def assign_submitted_upload(
        %{assigns: %{current_user: %{id: user_id}, uploading_file_for_record: %{id: record_id}}} =
          socket,
        pid
      ) do
  ...
  def reload_subscription_storage_and_allow_upload(
        %{
          assigns: %{
            current_subscription: %{
              id: subscription_id,
              deer_files_limit: total_files_limit,
    ...
    case socket.assigns do
      %{uploads: %{deer_file: %{entries: []}}} ->
        allow_deer_file_upload_or_overwrite_existing(socket, files_left, space_left)

      %{uploads: %{deer_file: %{entries: entries}}} ->
        case any_entry_started_upload?(entries) do
          true ->
            if limit_is_exceeded?(socket, files_left, space_left),
              do: raise("limit exceeded"),
              else: socket
  ...
  def assign_upload_result(%{assigns: %{upload_results: results}} = socket, message),
    do: assign(socket, :upload_results, [message | results])

  def assign_upload_result(socket, message), do: assign(socket, :upload_results, [message])

  def cancel_all_uploads_if_limit_is_exceeded(
        %{assigns: %{uploads: %{deer_file: %{max_entries: files, max_file_size: size}}}} = socket
      ) do
  ...
  def limit_is_exceeded?(
        %{assigns: %{uploads: %{deer_file: deer_file_uploads}}} = _socket,
        files_left,
        space_left
      ) do
    {files_count, size} =
      Enum.reduce(deer_file_uploads.entries, {0, 0}, fn entry, {files_count, size} ->
  ...
  def cancel_all_entries_in_socket(socket) do
    ...
    Enum.reduce(in_progress, socket, fn entry, socket_acc ->
      cancel_upload(socket_acc, :deer_file, entry.ref)
    end)
  end
