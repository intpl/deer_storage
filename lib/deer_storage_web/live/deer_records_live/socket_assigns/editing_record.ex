defmodule DeerStorageWeb.DeerRecordsLive.Index.SocketAssigns.EditingRecord do
  ...
  import DeerStorage.DeerRecords, only: [change_record: 3, update_record: 3]

  def assign_closed_editing_record(socket),
    do:
      assign(socket,
        editing_record: nil,
  ...
  def assign_editing_record_after_update(
        %{
          assigns: %{
            editing_record: %{data: %{id: record_id}} = old_editing_record_changeset,
  ...
  def assign_editing_record_after_delete(%{assigns: %{editing_record: nil}} = socket, _),
    do: socket
  ...
  def assign_opened_edit_record_modal(
        %{
          assigns: %{
    ...
    [record, _] =
      Enum.find(opened_records, fn [record, _connected_records] -> record.id == id end)
  ...
  def assign_saved_editing_record(
        %{
          assigns: %{
  ...
  def copy_editing_record_to_new_record(
        %{assigns: %{editing_record: %Ecto.Changeset{} = editing_record, new_record: nil}} =
          socket
      ) do
    socket |> assign(new_record: editing_record) |> assign_closed_editing_record
  end

  defp assign_error_editing_record_removed(socket),
    do: assign(socket, editing_record_has_been_removed: true)
end
