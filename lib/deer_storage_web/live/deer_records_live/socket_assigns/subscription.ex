defmodule DeerStorageWeb.DeerRecordsLive.Index.SocketAssigns.Subscription do
  alias DeerStorage.UserAvailableSubscriptionLinks.UserAvailableSubscriptionLink
  alias DeerStorage.Repo

  import DeerStorageWeb.DeerRecordsLive.Index.SocketAssigns.NewConnectedRecord,
  ...
  def assign_table_or_redirect_to_dashboard!(
  ...
  def assign_subscription_if_available_subscription_link_exists!(socket, _user_id, nil),
    do: socket
  ...
  def assign_updated_subscription(
        %{assigns: %{editing_record: editing_record, new_record: new_record, table_id: table_id}} =
          socket,
        subscription
      ) do
  ...
  def assign_subscription(socket, subscription) do
    assign(socket,
      ...
      current_subscription_deer_records_per_table_limit:
  ...
  defp maybe_assign_record_changeset(
         %{assigns: %{new_record_connecting_with_record_id: record_id}} = socket,
         :new_record,
         subscription,
         %{changes: %{deer_table_id: table_id}} = record
       )
  ...
  defp maybe_assign_record_changeset(
         ...
         assign_key,
         subscription,
         %{data: %{deer_table_id: table_id}} = record
       ) do
  ...
  defp maybe_reset_new_record_if_connecting(
         %{
           assigns: %{
             current_subscription_tables: tables,
