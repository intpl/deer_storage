defmodule DeerStorageWeb.DeerDashboardLive.Index do
  ...
  import DeerStorageWeb.LiveHelpers,
    only: [
      cached_counts: 1,
      is_expired?: 1,
      keys_to_atoms: 1,
  ...
  import DeerStorage.Subscriptions.DeerTable,
    only: [
      add_empty_column: 1,
  ...
  def mount(
    ...
    if connected?(socket) do
      ...
      PubSub.subscribe(DeerStorage.PubSub, "user_#{user.id}")
  ...
  def handle_event(
        "toggle_edit_subscription_name",
        %{},
        %{assigns: %{editing_subscription_name: bool}} = socket
      ) do
  ...
  def handle_event("add_table", %{}, %{assigns: %{current_subscription: subscription}} = socket) do
    case create_deer_table!(subscription, gettext("Example table"), [gettext("Example column 1")]) do
      {:error, subscription_changeset} ->
        invalid_changeset =
          subscription_changeset.changes.deer_tables |> Enum.find(fn dt -> dt.valid? == false end)
      ...
      {:ok, updated_subscription} ->
        ...
        PubSub.subscribe(DeerStorage.PubSub, "records_counts:#{created_table.id}")
  ...
  # {:noreply, assign(socket, :editing_table_id, nil)}

  def handle_event("cancel_table_edit", _, socket),
    do: {:noreply, assign(socket, editing_table_id: nil, editing_table_changeset: nil)}

  def handle_event(
        "validate_table_edit",
        %{"deer_table" => attrs},
        %{assigns: %{editing_table_changeset: ch}} = socket
      ) do
    {deer_table_attrs, _id} = attrs_to_deer_table(attrs)

    {:noreply,
     socket |> assign(editing_table_changeset: DeerTable.changeset(ch, deer_table_attrs))}
  end
  ...
  def handle_event(
    ...
    case update_deer_table!(subscription, deer_table_id, deer_table_attrs) do
      {:error, subscription_changeset} ->
        invalid_changeset =
          subscription_changeset.changes.deer_tables
          |> Enum.find(fn dt -> dt.data.id == deer_table_id end)
  ...
  def handle_event(
    ...
    {:noreply,
     socket |> assign(editing_table_changeset: move_column_to_index(ch, index, index - 1))}
  end
  ...
  def handle_event(
    ...
    changeset =
      ...
      |> Enum.find(fn dt -> dt.id == table_id end)
      |> DeerTable.changeset(%{})

    {:noreply,
     socket
     |> assign(
       editing_table_id: table_id,
       editing_table_changeset: changeset,
       editing_subscription_name: false
     )}
  end
  ...
  def handle_event(
    ...
    {_title, _description, tables} = DeerStorage.DeerTablesExamples.show(key)
      ...
      {:error, _subscription_changeset} ->
        {:noreply,
         assign(socket,
           displayed_error:
             gettext(
               "Your database limits do not allow to use this template. Please contact the Administrator to exceed limits."
             )
         )}
  ...
  def handle_event("reset_displayed_error", _, socket),
    do: {:noreply, assign(socket, displayed_error: nil)}
  ...
  def handle_event(
        "submit_upload",
        _,
        %{assigns: %{current_subscription: subscription, current_user: user}} = socket
      ) do
    pid = self()

    consume_uploaded_entries(socket, :csv_file, fn %{path: path},
  ...
  def handle_event("cancel_upload_entry", %{"ref" => ref}, socket),
    do: {:noreply, cancel_upload(socket, :csv_file, ref)}

  def handle_event("clear_csv_importer_messages", _, socket),
    do: {:noreply, assign(socket, :csv_importer_messages, [])}

  def handle_info(
        {:subscription_updated, %{deer_tables: new_tables} = subscription},
        %{assigns: %{current_subscription: %{deer_tables: old_tables}}} = socket
      ) do
  ...
  def handle_info(
        ...
        %{assigns: %{cached_counts: cached_counts}} = socket
      ) do
  ...
  def handle_cast(
        ...
        %{assigns: %{csv_importer_messages: messages}} = socket
      ) do
    {:noreply, assign(socket, csv_importer_messages: messages ++ [{:error, msg}])}
  end

  def handle_cast(
        {:csv_importer_info, msg},
        %{assigns: %{csv_importer_messages: messages}} = socket
      ) do
  ...
  def handle_params(
    ...
    case connected?(socket) do
          ...
          false ->
            {
              ...
              |> assign(
                ...
                storage_limit_kilobytes: subscription.storage_limit_kilobytes,
                subscription_deer_tables_limit: subscription.deer_tables_limit,
                subscription_deer_records_per_table_limit:
                  subscription.deer_records_per_table_limit,
                subscription_deer_columns_per_table_limit:
      ...
      false ->
        {:noreply,
         socket |> assign(current_subscription_name: "", current_subscription_tables: nil)}
    end
  end

  def handle_params(_, _, socket), do: {:noreply, socket}

  def render(%{current_subscription_tables: []} = assigns),
    do: DeerStorageWeb.DeerDashboardView.render("examples_index.html", assigns)

  def render(assigns),
    do: DeerStorageWeb.DeerDashboardView.render("editable_deer_tables.html", assigns)

  defp handle_cache_for_new_tables(
         %{assigns: %{cached_counts: cached_counts}} = socket,
         old_tables,
         new_tables
       ) do
    ...
    new_cached_counts =
      Enum.reduce(new_tables, cached_counts, fn table_id, inner_cached_counts ->
        Map.merge(inner_cached_counts, %{
          table_id => DeerCache.RecordsCountsCache.fetch_count(table_id)
        })
      end)
  ...
  defp assign_examples_if_no_subscription_tables(
         %{assigns: %{current_subscription_tables: []}} = socket
       ) do
    assign(socket, all_listed_examples: DeerStorage.DeerTablesExamples.list_examples())
  end
