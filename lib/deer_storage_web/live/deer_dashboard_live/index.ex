defmodule DeerStorageWeb.DeerDashboardLive.Index do
  ...
  import DeerStorageWeb.LiveHelpers, only: [
    cached_counts: 1,
    is_expired?: 1,
    keys_to_atoms: 1,
  ...
  import DeerStorage.Subscriptions, only: [
    update_deer_table!: 3,
    create_deer_table!: 3,
  ...
  import DeerStorage.Subscriptions.DeerTable, only: [
    add_empty_column: 1,
  ...
  def mount(_params, %{"deer_storage_auth" => _token, "current_subscription_id" => nil}, socket), do: {:ok, push_redirect(socket, to: "/registration/edit")}
  def mount(_params, %{"deer_storage_auth" => token, "current_subscription_id" => subscription_id} = session, socket) do
  ...
  def handle_event("submit_upload", _, %{assigns: %{current_subscription: subscription, current_user: user}} = socket) do
    ...
    consume_uploaded_entries(socket, :csv_file, fn %{path: path}, %{client_name: original_filename, uuid: uuid} ->
      ...
      spawn(DeerStorage.CsvImporter, :run!, [pid, subscription, user, tmp_path, original_filename, uuid, true])
    end)
  ...
  def render(%{current_subscription_tables: []} = assigns), do: DeerStorageWeb.DeerDashboardView.render("examples_index.html", assigns)
  def render(assigns), do: DeerStorageWeb.DeerDashboardView.render("editable_deer_tables.html", assigns)
