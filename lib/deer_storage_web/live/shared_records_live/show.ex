defmodule DeerStorageWeb.SharedRecordsLive.Show do
  ...
  import DeerStorageWeb.DeerRecordsLive.Index.SocketAssigns.Helpers, only: [
  ...
  import DeerStorageWeb.DeerRecordsLive.Index.SocketAssigns.UploadingFiles, only: [
  ...
  import DeerStorage.DeerRecords, only: [change_record: 3, update_record: 3, delete_file_from_record!: 3, ensure_deer_file_exists_in_record!: 2]
    ...
    case connected?(socket) do
      true ->
        ...
        redirect_if_expired(socket, subscription, fn ->
          shared_record = SharedRecords.get_record!(subscription_id, shared_record_uuid) |> DeerStorage.Repo.preload(:deer_record)
  ...
  def handle_event("submit_upload", _, %{assigns: %{is_editable: true, deer_record: %{id: record_id}, shared_record: %{created_by_user_id: user_id}}} = socket) do
    ...
    consume_uploaded_entries(socket, :deer_file, fn %{path: path}, %{client_name: original_filename, uuid: uuid} ->
      ...
      spawn(DeerStorage.Services.UploadDeerFile, :run!, [pid, tmp_path, original_filename, record_id, user_id, uuid])
    end)
