defmodule DeerStorageWeb.SharedRecordsLive.Show do
  ...
  import DeerStorageWeb.DeerRecordsLive.Index.SocketAssigns.UploadingFiles,
    only: [
  ...
  alias DeerStorageWeb.DeerRecordView
  ...
  def handle_params(
        %{"shared_record_uuid" => shared_record_uuid, "subscription_id" => subscription_id},
        _,
        socket
      ) do
    case connected?(socket) do
      true ->
        ...
        redirect_if_expired(socket, subscription, fn ->
          shared_record =
            SharedRecords.get_record!(subscription_id, shared_record_uuid)
            |> DeerStorage.Repo.preload(:deer_record)
          ...
          PubSub.subscribe(DeerStorage.PubSub, "shared_records_invalidates:#{deer_record.id}")
  ...
  def handle_event("close_edit", _, %{assigns: %{is_editable: true}} = socket),
    do: {:noreply, assign(socket, editing_record: nil, old_editing_record: nil)}

  def handle_event("close_upload_file_modal", _, %{assigns: %{is_editable: true}} = socket),
    do:
      {:noreply,
       assign(socket, uploading: false, upload_results: []) |> cancel_all_entries_in_socket}

  def handle_event("show_upload_file_modal", _, %{assigns: %{is_editable: true}} = socket),
    do:
      {:noreply,
       socket |> assign(:uploading, true) |> reload_subscription_storage_and_allow_upload}
  ...
  def handle_event(
        ...
        %{
          assigns: %{
            is_editable: true,
            deer_record: %{id: record_id},
            shared_record: %{created_by_user_id: user_id}
          }
        } = socket
      ) do
    pid = self()

    consume_uploaded_entries(socket, :deer_file, fn %{path: path},
                                                    %{client_name: original_filename, uuid: uuid} ->
  ...
  def handle_event(
        ...
        %{
          assigns: %{
            deer_record: %{deer_table_id: table_id} = record,
            current_subscription: subscription,
            is_editable: true
          }
        } = socket
  ...
  def handle_event(
    ...
    {:noreply,
     assign(socket,
       editing_record:
         change_record(subscription, record, atomize_and_merge_table_id_to_attrs(attrs, table_id))
     )}
  end
  ...
  def handle_event(
        "preview_file",
        %{"file-id" => file_id},
        %{assigns: %{deer_record: record}} = socket
      ) do
    deer_file = ensure_deer_file_exists_in_record!(record, file_id)

    DeerRecordView.mimetype_is_previewable?(deer_file.mimetype) ||
      raise "invalid preview requested"

    {:noreply, assign(socket, :preview_deer_file, deer_file)}
  end

  def handle_event("next_file_gesture", _, socket),
    do: send(self(), :preview_next_file) && {:noreply, socket}

  def handle_event("previous_file_gesture", _, socket),
    do: send(self(), :preview_previous_file) && {:noreply, socket}

  def handle_info(:close_preview_modal, socket),
    do: {:noreply, assign(socket, :preview_deer_file, nil)}

  def handle_info(
        :preview_previous_file,
        %{
          assigns: %{
            deer_record: %{deer_files: deer_files},
  ...
  def handle_info(
        {:batch_record_delete, deleted_record_ids},
  ...
  def handle_info({:record_update, _}, socket),
    do: {:noreply, maybe_reload_and_overwrite_deer_file_upload(socket)}

  def handle_info(
        {:subscription_updated, subscription},
        %{assigns: %{deer_record: %{deer_table_id: deer_table_id}}} = socket
      ) do
    redirect_if_expired(socket, subscription, fn ->
      case DeerRecordView.deer_table_from_subscription(subscription, deer_table_id) do
        ...
        _ ->
          {:noreply,
           assign(socket, current_subscription: subscription)
           |> maybe_reload_and_overwrite_deer_file_upload}
      end
    end)
  end
  ...
  defp assign_editing_deer_record_as_old_record(
         %{
           assigns: %{
             editing_record:
               %{data: %{id: record_id, deer_table_id: table_id}} = old_editing_record_changeset,
             current_subscription: subscription
           }
         } = socket,
         %{id: record_id} = record_in_database
       ) do
    ch =
      overwrite_deer_fields(
        record_in_database,
        fetch_field!(old_editing_record_changeset, :deer_fields)
      )
    ...
    case connected_records_or_deer_files_changed?(
           old_editing_record_changeset,
           record_in_database
         ) do
      true ->
        socket
      ...
      false ->
        assign(socket,
          old_editing_record:
            change_record(subscription, record_in_database, %{deer_table_id: table_id})
        )
    end
  end
  ...
  defp maybe_close_preview_modal(%{assigns: %{preview_deer_file: nil}} = socket, _updated_record),
    do: socket

  defp maybe_close_preview_modal(
         %{assigns: %{preview_deer_file: %{id: preview_deer_file_id}}} = socket,
         updated_record
       ) do
