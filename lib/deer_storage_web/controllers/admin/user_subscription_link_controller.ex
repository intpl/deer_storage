defmodule DeerStorageWeb.Admin.UserSubscriptionLinkController do
  ...
  def delete(conn, %{"id" => user_id, "subscription_id" => subscription_id} = params),
    do: delete_user_subscription_link(conn, params, user_id, subscription_id)

  def delete(conn, %{"user_id" => user_id, "id" => subscription_id} = params),
    do: delete_user_subscription_link(conn, params, user_id, subscription_id)
  ...
  defp delete_user_subscription_link(conn, params, user_id, subscription_id) do
    ...
    Users.remove_subscription_link_and_maybe_change_last_used_subscription_id(
      user,
      subscription_id
    )
  ...
  defp path_to_redirect(conn, %{"subscription_id" => id, "redirect_back_to" => "subscription"}),
    do: Routes.admin_subscription_path(conn, :show, id)

  defp path_to_redirect(conn, %{"user_id" => id, "redirect_back_to" => "user"}),
    do: Routes.admin_user_path(conn, :show, id)
end
