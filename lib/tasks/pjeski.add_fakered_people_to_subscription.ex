defmodule Mix.Tasks.Pjeski.AddFakeredPeopleToSubscription do
  use Mix.Task
  ...
  def run([subscription_id, count]) do
    ...
    find_fake_people = fn deer_tables -> Enum.find(deer_tables, fn dt -> dt.name == "Fake People" end) end
    ...
    {subscription, deer_table} = case find_fake_people.(subscription.deer_tables) do
                   nil ->
                     {:ok, subscription} = create_deer_table!(subscription, "Fake People",
                     ["Name", "Age", "City", "Country", "Mobile phone"])

                     {subscription, find_fake_people.(subscription.deer_tables)}
                   fake_people -> {subscription, fake_people}
                 end
