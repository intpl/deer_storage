defmodule Mix.Tasks.DeerStorage.AddFakeredCarsToSubscription do
  use Mix.Task
  ...
  def run([subscription_id, count]) do
    ...
    find_fake_cars = fn deer_tables ->
      Enum.find(deer_tables, fn dt -> dt.name == "Fake cars" end)
    end
    ...
    {subscription, deer_table} =
      case find_fake_cars.(subscription.deer_tables) do
        nil ->
          {:ok, subscription} =
            create_deer_table!(subscription, "Fake cars", [
              "Make and model",
              "Year",
              "Transmission",
              "Fuel type",
