defmodule DeerStorage.CsvImporter do
  require Logger
  ...
  def run!(pid, subscription, user, path, filename, random_name, remove_file? \\ false) do
    ...
    log_info(
      pid,
      gettext(
        "Starting importer for file named '%{filename}' in the background (don't close this window to receive messages)...",
        filename: filename
      )
    )
  ...
  defp lock_and_update_subscription(
         {:ok,
          %{subscription: %{id: subscription_id}, headers: headers, random_name: random_name} =
            assigns}
       ) do
  ...
  defp prepare_table_data(
         {:ok, %{subscription: %{deer_tables: deer_tables}, random_name: random_name} = assigns}
       ) do
  ...
  defp prepare_records(
    ...
    {result, _count} =
      Enum.reduce(stream, {[], 0}, fn item, {list, count} ->
        if count < limit do
          case item do
            {:ok, fields_list} ->
              case prepare_record_map(
                     fields_list,
                     columns_ids,
                     table_id,
                     subscription.id,
            ...
            {:error, message} ->
              throw(gettext("CSV parser error: %{message}", message: message))
          end
        else
  ...
  defp insert_records_and_notify_subscribers(
         {:ok,
          %{
            caller_pid: pid,
            records_maps: records_maps,
            subscription: subscription,
            table_id: table_id,
            filename: filename
          } = assigns}
    ...
    total_inserted_count =
      Enum.reduce(maps_chunks, 0, fn chunk, acc ->
    ...
    log_info(
      pid,
      gettext("Inserted %{count} records from file '%{filename}'",
        count: total_inserted_count,
        filename: filename
      )
    )
  ...
  defp rename_subscription_table_to_filename(
         {:ok,
          %{
    ...
    name =
      case String.length(filename) do
        # 50 characters
        len when len > 50 -> String.slice(filename, 0..46) <> "..."
        # LV will not allow files without .csv extension, so skip handling files with name having less than 3 characters
        _len -> filename
      end

    {:ok, new_subscription} =
      Subscriptions.update_deer_table!(subscription, target_table_id, %{name: name})
  ...
  defp ensure_subscription_table_limit_is_not_exceeded(
         {:ok, %{subscription: %{deer_tables: deer_tables, deer_tables_limit: limit}}}
       )
       when length(deer_tables) > limit, do: {:error, gettext("Exceeded table limit")}
  ...
  defp prepare_record_map(fields_list, columns_ids, table_id, subscription_id, user_id, timestamp) do
    deer_fields =
      Enum.zip(fields_list, columns_ids)
      |> Enum.map(fn {content, column_id} ->
  ...
  defp log_error(pid, filename, [validation_error | _]) when is_tuple(validation_error) do
    if unquote(Mix.env() != :test),
      do: Logger.error("CSV import #{filename}: Subscription limits validation failed")

    GenServer.cast(
      pid,
      {:csv_importer_error,
       gettext("Your database limits don't allow to import file named '%{filename}'.",
         filename: filename
       )}
    )
  end
  ...
  defp log_error(pid, filename, _) do
    ...
    GenServer.cast(
      pid,
      {:csv_importer_error,
       gettext("Unknown error occurred when importing '%{filename}'.", filename: filename)}
    )
  end
