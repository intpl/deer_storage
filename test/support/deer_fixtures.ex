defmodule Pjeski.DeerFixtures do
  ...
  def create_valid_subscription_with_tables(tables_count \\ 1, columns_count \\ 1) do
    deer_tables = Enum.map(1..tables_count, fn _ ->
      deer_columns = Enum.map(1..columns_count, fn _ -> %{name: Faker.Commerce.product_name_product()} end)
    ...
    {:ok, subscription_with_deer} = Subscriptions.update_subscription_deer(subscription, %{deer_tables: deer_tables})

    subscription_with_deer
  end

  def create_valid_records_for_subscription(%Subscription{deer_tables: deer_tables} = subscription, records_per_table_count \\ 1) do
