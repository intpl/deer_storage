defmodule DeerStorageWeb.LocalePlugTest do
  use DeerStorageWeb.ConnCase
  import DeerStorage.Fixtures

  import DeerStorage.Test.SessionHelpers,
    only: [assign_locale_to_session: 2, assign_user_to_session: 2]
  ...
  describe "when the accept-language header is \"pl\" and there is no locale in session" do
    test "it puts \"pl\" locale" do
      conn =
        ...
        |> Plug.Conn.put_req_header("accept-language", "pl-PL, pl;q=0.8, nl;q=0.9, en;q=0.7")
        |> get("/")
  ...
  describe "when the accept-language header is invalid and there is no locale in session" do
    test "it puts default locale" do
      conn =
        Phoenix.ConnTest.build_conn(:get, "/")
        |> Plug.Conn.put_req_header("accept-language", "INVALID 123, 2, 2")
        |> get("/")
