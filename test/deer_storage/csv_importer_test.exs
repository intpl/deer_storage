defmodule DeerStorage.CsvImporterTest do
  ...
  describe "run!" do
    setup do
      ...
      all_records_for = fn (subscription_id) -> DeerRecord |> where([dr], dr.subscription_id == ^subscription_id) |> DeerStorage.Repo.all end
    ...
    test "imports polish_people.csv correctly", %{subscription: subscription, user: user, all_records_for: all_records_for} do
      assert {:ok, :ok} = CsvImporter.run!(self(), subscription, user, @polish_people_csv, "test.csv", "XYZ")
      ...
      table = DeerStorage.Subscriptions.get_subscription!(subscription.id).deer_tables |> Enum.find(fn dt -> dt.id == table_id end)
