defmodule DeerStorage.CsvImporterTest do
  use DeerStorage.DataCase
  import DeerStorage.Fixtures

  alias DeerStorage.CsvImporter
  ...
  describe "run!" do
    setup do
      ...
      all_records_for = fn subscription_id ->
        DeerRecord
        |> where([dr], dr.subscription_id == ^subscription_id)
        |> DeerStorage.Repo.all()
      end

      {:ok,
       user: user, subscription: user.last_used_subscription, all_records_for: all_records_for}
    end

    test "imports polish_people.csv correctly", %{
      subscription: subscription,
      user: user,
      all_records_for: all_records_for
    } do
      assert {:ok, :ok} =
               CsvImporter.run!(self(), subscription, user, @polish_people_csv, "test.csv", "XYZ")
      ...
      table =
        DeerStorage.Subscriptions.get_subscription!(subscription.id).deer_tables
    ...
    test "returns error when deer_tables_limit is being exceeded", %{
      subscription: subscription,
      user: user,
      all_records_for: all_records_for
    } do
      {:ok, subscription} =
        subscription |> Subscriptions.admin_update_subscription(%{deer_tables_limit: 0})
    ...
    test "returns error when deer_columns_per_table_limit is being exceeded", %{
      subscription: subscription,
      user: user,
      all_records_for: all_records_for
    } do
      {:ok, subscription} =
        subscription
        |> Subscriptions.admin_update_subscription(%{deer_columns_per_table_limit: 1})

      # regression test
      {:ok, subscription} =
        Subscriptions.create_deer_table!(subscription, "example table", ["first column"])
    ...
    test "returns error when deer_records_per_table_limit is being exceeded", %{
      subscription: subscription,
      user: user,
      all_records_for: all_records_for
    } do
      {:ok, subscription} =
        subscription
        |> Subscriptions.admin_update_subscription(%{deer_records_per_table_limit: 0})
