defmodule DeerStorage.DeerRecordsTest do
  ...
  describe "get_record!/2" do
    ...
    test "valid id and subscription_id", %{
      subscription: subscription,
      records_for_subscription: records
    } do
  ...
  describe "update_record/3" do
    ...
    test "valid params -> change order", %{
      subscription: subscription,
      target_record: target_record,
      first_column_id: first_column_id,
      second_column_id: second_column_id,
      third_column_id: third_column_id
    } do
      {:ok, deer_record} =
        DeerRecords.update_record(
          subscription,
  ...
  describe "create_record/2" do
    ...
    test "record/fields with identical deer_column_id cannot be saved", %{
      subscription: %{deer_tables: [deer_table]} = subscription
    } do
      assert {:error, _} =
               DeerRecords.create_record(
                 subscription,
  ...
  describe "connect_records/3 and disconnect_records!/3" do
    ...
    test "connect and disconnect records", %{
      subscription: subscription,
      records_for_subscription: records
    } do
      [r1, r2] = records
    ...
    test "cannot connect record to itself", %{
      subscription: subscription,
      records_for_subscription: [record, _]
    } do
      assert_raise RuntimeError, fn ->
        DeerRecords.connect_records!(record, record, subscription.id)
      end
    end
  end
end
