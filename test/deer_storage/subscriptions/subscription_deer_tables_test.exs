defmodule DeerStorage.SubscriptionDeerTablesTest do
  ...
  describe "update_deer_table!/3" do
    ...
    test "change order of columns", %{
      subscription: subscription,
      target_table: target_table,
      target_column: target_column,
      second_column_map: second_column_map
    } do
      {:ok, %{deer_tables: updated_deer_tables} = updated_subscription} =
        update_deer_table!(
          subscription,
          target_table.id,
          %{
    ...
    test "change name of one column leaving rest untouched", %{
      subscription: subscription,
      target_table: target_table,
      target_column: target_column,
      second_column_map: second_column_map
      ...
      assert List.last(updated_target_table.deer_columns) ==
               List.last(List.first(subscription.deer_tables).deer_columns)

      assert List.last(updated_subscription.deer_tables) == List.last(subscription.deer_tables)
    end

    test "validates minimal length of deer column", %{
      subscription: subscription,
      target_table: target_table,
      target_column: target_column,
      second_column_map: second_column_map
    ...
    test "validates maximum length of deer column", %{
      subscription: subscription,
      target_table: target_table,
      target_column: target_column,
      second_column_map: second_column_map
        ...
        update_deer_table!(
          ...
          %{
            ...
            deer_columns: [
              %{
                id: target_column.id,
                name: "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx1"
              },
              second_column_map
            ]
          }
    ...
    test "does not change params table id when invalid", %{
      subscription: subscription,
      target_table: target_table,
      target_column: target_column,
      second_column_map: second_column_map
      ...
      {:ok, updated_subscription} =
        update_deer_table!(subscription, "hacked", %{
          id: target_table.id,
          deer_columns: [%{id: target_column.id, name: "new value"}, second_column_map]
        })

      assert subscription == updated_subscription
    end

    test "does not change attrs table id when invalid", %{
      subscription: subscription,
      target_table: target_table,
      target_column: target_column,
      second_column_map: second_column_map
      ...
      assert {:error, _} =
               update_deer_table!(subscription, target_table.id, %{
                 id: "hacked",
                 deer_columns: [%{id: target_column.id, name: "new value"}, second_column_map]
               })
    end

    test "does not change column id when invalid", %{
      subscription: subscription,
      target_table: target_table,
      second_column_map: second_column_map
    } do
      assert {:error, _} =
               update_deer_table!(subscription, target_table.id, %{
                 id: target_table.id,
                 deer_columns: [%{id: "hacked id", name: "hacked name"}, second_column_map]
               })
    end
  end
end
