defmodule DeerStorage.DeerRecordsSearchTest do
  ...
  describe "search_records/4" do
    test "composing" do
      ...
      # therefore total records count is 6
      create_valid_records_for_subscription(subscription, 3)

      result = DeerRecordsSearch.search_records(subscription.id, deer_table.id, ["Content"], 1)
      assert length(result) == 3

      result2 =
        DeerRecordsSearch.search_records(subscription.id, deer_table.id, ["Content", "1"], 1)

      assert length(result2) == 1

      result3 =
        DeerRecordsSearch.search_records(
          subscription.id,
          deer_table.id,
